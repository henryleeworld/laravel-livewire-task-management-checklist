...
class MenuService
{
    ...
    public function get_menu(): array
    {
        ...
        if (!auth()->user()->is_admin) {
            ...
            $user_tasks_menu = [
                'my_day' => [
                    'name' => trans('admin.manage_checklists.tasks.my_day.title'),
                    'icon' => 'sun',
                    'tasks_count' => $user_tasks->whereNotNull('added_to_my_day_at')->count()
                ],
                'important' => [
                    'name' => trans('admin.manage_checklists.tasks.important.title'),
                    'icon' => 'star',
                    'tasks_count' => $user_tasks->where('is_important', 1)->count()
                ],
                'planned' => [
                    'name' => trans('admin.manage_checklists.tasks.planned.title'),
                    'icon' => 'calendar',
