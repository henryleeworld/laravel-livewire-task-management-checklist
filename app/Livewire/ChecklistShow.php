    ...
    public function render()
    {
        if (is_null($this->list_type)) {
            ...
            switch ($this->list_type) {
                case 'my_day':
                    $this->list_name = trans('admin.manage_checklists.tasks.my_day.title');
                    $this->user_tasks = Task::where('user_id', auth()->id())->whereNotNull('added_to_my_day_at')->get();
                    break;
                case 'important':
                    $this->list_name = trans('admin.manage_checklists.tasks.important.title');
                    $this->user_tasks = Task::where('user_id', auth()->id())->where('is_important', 1)->get();
                    break;
                case 'planned':
                    $this->list_name = trans('admin.manage_checklists.tasks.planned.title');
