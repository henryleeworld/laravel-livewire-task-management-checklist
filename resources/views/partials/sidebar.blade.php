<div class="c-sidebar c-sidebar-dark c-sidebar-fixed c-sidebar-lg-show" id="sidebar">
    ...
    <ul class="c-sidebar-nav">
        @if (auth()->user()->is_admin)
            <li class="c-sidebar-nav-title">{{ trans('admin.manage_checklists.title') }}</li>
                        ...
                        <li class="c-sidebar-nav-item">
                            <a class="c-sidebar-nav-link" style="padding: 1rem .5rem .5rem 76px"
                               href="{{ route('admin.checklist_groups.checklists.create', $group) }}">
                                ...
                                {{ trans('admin.manage_checklists.checklist.create.title') }}</a>
                        </li>
                    </ul>
                </li>
            @endforeach
            <li class="c-sidebar-nav-item c-sidebar-nav-dropdown">
                <a class="c-sidebar-nav-link" href="{{ route('admin.checklist_groups.create') }}">
                    ...
                    {{ trans('admin.manage_checklists.checklist_group.create.title') }}</a>
            </li>

            <li class="c-sidebar-nav-title">{{ trans('admin.pages.title') }}</li>
            ...
            <li class="c-sidebar-nav-title">{{ trans('admin.manage_data.title') }}</li>
            <li class="c-sidebar-nav-item c-sidebar-nav-dropdown">
                <a class="c-sidebar-nav-link"
                   href="{{ route('admin.users.index') }}">
                    <svg class="c-sidebar-nav-icon">
                        <use xlink:href="{{ asset('vendors/@coreui/icons/svg/free.svg#cil-puzzle') }}"></use>
                    </svg> {{ trans('admin.manage_data.users.title') }}
                </a>
            </li>
        @else
            ...
            @foreach ($user_menu as $group)
                <li class="mt-2 c-sidebar-nav-item c-sidebar-nav-dropdown c-show">
                    <a class="c-sidebar-nav-link c-sidebar-nav-dropdown-toggle" href="#">
                        ...
                        @if ($group['is_new'])
                            <span class="badge badge-info">{{ trans('admin.manage_data.menus.badge.new') }}</span>
                        @elseif ($group['is_updated'])
                            <span class="badge badge-info">{{ trans('admin.manage_data.menus.badge.upd') }}</span>
                        @endif
                    </a>
                    <ul class="c-sidebar-nav-dropdown-items">
                        @foreach ($group['checklists'] as $checklist)
